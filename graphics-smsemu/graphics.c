...
void vdp_plotGG(int32_t x, int32_t y, uint8_t c)
{
    ...
    // Need x+2 valid, and y within 0..319
    if ((uint32_t)x > 477u || (uint32_t)y > 319u) return;

    base = PROJ_VRAM + y;

    // 4 vertical pixels packed (column-major: y,y+1,y+2,y+3)
    cc = ((uint32_t)c << 24) | ((uint32_t)c << 16) | ((uint32_t)c << 8) | (uint32_t)c;

    uint8_t *p1 = base + dxf[x];
    uint8_t *p2 = base + dxf[x + 1];
    uint8_t *p3 = base + dxf[x + 2];

    // Fast path: y <= 316, can write 4 bytes safely
    if ((uint32_t)y <= 316u) {
        *(uint32_t *)p1 = cc;
        *(uint32_t *)p2 = cc;
        *(uint32_t *)p3 = cc;
    ...
    // Tail path: last 3 rows (y=317..319), write only what's in range
    // rowsLeft = 320 - y  (1..3)
    uint32_t rowsLeft = 320u - (uint32_t)y;

    // Always write first byte
    p1[0] = c; p2[0] = c; p3[0] = c;
    if (rowsLeft >= 2u) { p1[1] = c; p2[1] = c; p3[1] = c; }
    if (rowsLeft >= 3u) { p1[2] = c; p2[2] = c; p3[2] = c; }
}
