...
#define smsVRAMLocation duunmap_textureid[0]        		// 16KB


extern uint8_t smsCYC_00[256];//		(duunmap_types[0] + 0)
extern uint8_t smsCYC_ED[256];//		(duunmap_types[0] + 256 )			// 256b
extern uint8_t smsCYC_DDFD[256];//		(duunmap_types[0] + 256 + 256) 		// 256b
extern uint8_t smsCYC_CB[256];//		(duunmap_types[0] + 256 + 256 + 256) // 256b

extern uint8_t smsCARTRam0[16384];// 	(textures[0] + (16 * 1024))	// 16KB
extern uint8_t smsCARTRam1[16384];// 	(textures[0] + (32 * 1024))	// 16KB
extern uint8_t smsSysRamBuff[16384];// 	(textures[0] + (48 * 1024))	// 16KB
// fwd
//struct SMS_Ports;
...
struct SN76489
{
    struct
    {
        ...
        float pwm;		// PWM Effect (ADDED NOT OFFICAL -;))
        bool pwmbit;	// which direction is the duty cycle going
    } tone[3];
