...
static void rc3dBuildPlaneTables(void)
{
    for (int y = 0; y < SCREEN_H; ++y) {
        ...
        if (d == 0) g_invRowDist[y] = 0.0f;
        else        g_invRowDist[y] = 1.0f / (float)d;
    }
...
static void rc3dBuildDefaultTextures(void)
{
    ...
    // clear the textures to base colours;
    for (i = 0; i < 256; ++i) {
    ...
    for(y = 0; y < RC3D_TEX_SIZE; ++y){
        for(x = 0; x < RC3D_TEX_SIZE; ++x){
            g_rc3dTextures[10].pix[tindex] = txt_brick1[tindex];
            g_rc3dTextures[11].pix[tindex] = txt_lava[tindex];
            g_rc3dTextures[12].pix[tindex] = txt_water[tindex];
            g_rc3dTextures[13].pix[tindex] = txt_grass[tindex];
            g_rc3dTextures[14].pix[tindex] = txt_burnedwood[tindex];
            tindex++;
        }
    }
...
static int findSectorForPoint(float x, float y)
{
    ...
    for (int i = 0; i < g_map->sectorCount; i++) {
        ...
        if (!pointInSector(x, y, i)) continue;
...
static int findBlockingWallInSector(
    float px, float py,
    float radius,
    int sectorIndex,
    int *outWallIndex
...
static int findBlockingWallAtPosition(
    float px, float py,
    float radius,
    int currentSector,
    int newSector,
    ...
    if (findBlockingWallInSector(px, py, radius, currentSector, outWallIndex)) {
        return 1;
    }

    if (newSector != currentSector && newSector >= 0) {
        if (findBlockingWallInSector(px, py, radius, newSector, outWallIndex)) {
            return 1;
        }
    }
...
{
    return findBlockingWallInSector(px, py, radius, sectorIndex, NULL);
}
...
static int canMoveToPosition(float px, float py, int newSector)
{
    ...
    if (positionHitsSolidWallsInSector(px, py, PLAYER_RADIUS, g_player.sector)) {
        return 0;
    }

    if (newSector != g_player.sector) {
        if (positionHitsSolidWallsInSector(px, py, PLAYER_RADIUS, newSector)) {
            return 0;
        }
    }
...
static int tryMovePlayerSliding(float moveX, float moveY)
{
    const float startX = g_player.x;
    const float startY = g_player.y;

    const float tryX = startX + moveX;
    const float tryY = startY + moveY;
    const int newSector = findSectorForPoint(tryX, tryY);

    if (canMoveToPosition(tryX, tryY, newSector)) {
        g_player.x = tryX;
        g_player.y = tryY;
...

    {
                ...
                &wallIndex))
        {
            ...
            if (wallLen > RC3D_EPSILON) {
                ...
                const float slideTryX = startX + slideX;
                const float slideTryY = startY + slideY;
                const int slideSector = findSectorForPoint(slideTryX, slideTryY);

                if (canMoveToPosition(slideTryX, slideTryY, slideSector)) {
                    g_player.x = slideTryX;
                    g_player.y = slideTryY;
    ...
    {
        const float xOnlyX = startX + moveX;
        const float xOnlyY = startY;
        const int xSector = findSectorForPoint(xOnlyX, xOnlyY);

        if (canMoveToPosition(xOnlyX, xOnlyY, xSector)) {
            g_player.x = xOnlyX;
            g_player.y = xOnlyY;
    ...
    {
        const float yOnlyX = startX;
        const float yOnlyY = startY + moveY;
        const int ySector = findSectorForPoint(yOnlyX, yOnlyY);

        if (canMoveToPosition(yOnlyX, yOnlyY, ySector)) {
            g_player.x = yOnlyX;
            g_player.y = yOnlyY;
...
static inline RC3D_WallHit findNearestWallInSector(
    ...
    for (int wallIndex = start; wallIndex < end; ++wallIndex) {
...

        {
            ...
            const float nx = sy;
            const float ny = -sx;
            if (((nx * rdx) + (ny * rdy)) >= 0.0f) continue;

            {
                const float denom = (rdx * sy) - (rdy * sx);
                if (denom > -RC3D_EPSILON && denom < RC3D_EPSILON) continue;
...

                            {
...

                                {
                                    ...
                                    for (int testIndex = nextStart; testIndex < nextEnd; ++testIndex) {
                                        ...
                                        if (tw->neighbour != prevSector) continue;
...
void rc3dInit(void)
{
    ...
    if (!g_planeTablesInit)  rc3dBuildPlaneTables();
    if (!g_rayTableInit)     rc3dBuildRayTable();
...
void rc3dUpdate(float dt, const uint8_t *keys, int mouseDx)
{
    ...
    if ((moveX != 0.0f) || (moveY != 0.0f)) {
        ...
        tryMovePlayerSliding(moveX, moveY);
    }
