...
// POINTERS to a area of ram that is not going to be used by SIDBOX OS since the SMS is literally running in its self.
__ATTR_RAM_TC uint8_t *CYC_00;
__ATTR_RAM_TC uint8_t *CYC_ED;
__ATTR_RAM_TC uint8_t *CYC_DDFD;
__ATTR_RAM_TC uint8_t *CYC_CB;

static const uint8_t DBCYC_00[0x100] = {
...
static const uint8_t DBCYC_ED[0x100] = {
...
static const uint8_t DBCYC_DDFD[0x100] = {
...
static const uint8_t DBCYC_CB[0x100] = {
    ...
    8,8,8,8,8,8,12,8,8,8,8,8,8,8,12,8,	// 0x40 - 0x4F (BIT instructions)
    8,8,8,8,8,8,12,8,8,8,8,8,8,8,12,8,	// 0x50 - 0x5F (BIT instructions)
    8,8,8,8,8,8,12,8,8,8,8,8,8,8,12,8,	// 0x60 - 0x6F (BIT instructions)
    8,8,8,8,8,8,12,8,8,8,8,8,8,8,12,8,	// 0x70 - 0x7F (BIT instructions)

    8,8,8,8,8,8,15,8,8,8,8,8,8,8,15,8,	// 0x80 - 0x8F (RES instructions)
    8,8,8,8,8,8,15,8,8,8,8,8,8,8,15,8,	// 0x90 - 0x9F (RES instructions)
    8,8,8,8,8,8,15,8,8,8,8,8,8,8,15,8,	// 0xA0 - 0xAF (RES instructions)
    8,8,8,8,8,8,15,8,8,8,8,8,8,8,15,8,	// 0xB0 - 0xBF (RES instructions)

    8,8,8,8,8,8,15,8,8,8,8,8,8,8,15,8,	// 0xC0 - 0xCF (SET instructions)
    8,8,8,8,8,8,15,8,8,8,8,8,8,8,15,8,	// 0xD0 - 0xDF (SET instructions)
    8,8,8,8,8,8,15,8,8,8,8,8,8,8,15,8,	// 0xE0 - 0xEF (SET instructions)
    8,8,8,8,8,8,15,8,8,8,8,8,8,8,15,8,	// 0xF0 - 0xFF (SET instructions)
};
...
static FORCE_INLINE bool calc_vflag_16(const uint8_t a, const uint8_t b, const uint8_t r) {
    ...
    return (((a ^ r) & (a ^ b)) & 0x8000) != 0;
}

static FORCE_INLINE void _ADD(struct SMS_Core *sms, uint16_t value) {
    uint16_t sum = REG_A + value;
    uint8_t result = (uint8_t)sum;

    FLAG_C  = sum > 0xFF;
    FLAG_N  = false;
    FLAG_V  = (((REG_A ^ result) & (REG_A ^ value)) & 0x80) != 0;
    FLAG_H  = ((REG_A ^ value ^ result) & 0x10) != 0;  // slightly faster half-carry calc
    FLAG_B3 = (result >> 3) & 1;
    FLAG_B5 = (result >> 5) & 1;
    FLAG_Z  = result == 0;
    ...
    FLAG_B3 = BIT_TST3(result);
    FLAG_B5 = BIT_TST5(result);
    FLAG_Z = result == 0;
    FLAG_S = result >> 7;
...
static FORCE_INLINE void _SUB(struct SMS_Core *sms, uint16_t value) {
    ...
    REG_A = result;
    */
    uint16_t diff = REG_A - value;
    uint8_t result = (uint8_t)diff;

    FLAG_C  = value > REG_A;
    FLAG_N  = true;
    FLAG_V  = (((REG_A ^ value) & (REG_A ^ result)) & 0x80) != 0;
    FLAG_H  = ((REG_A ^ value ^ result) & 0x10) != 0;  // Optimized half-carry
    FLAG_B3 = (result >> 3) & 1;
    FLAG_B5 = (result >> 5) & 1;
    FLAG_Z  = result == 0;
    FLAG_S  = (result & 0x80) != 0;
...
extern __ATTR_RAM_TC uint8_t parity8_lookup[256];
...
static FORCE_INLINE void _CP(struct SMS_Core *sms, const uint8_t value) {
    ...
    FLAG_V = calc_vflag_8(REG_A, ~value + 1, result);
    FLAG_H = (REG_A & 0xF) < (value & 0xF);
    FLAG_B3 = BIT_TST3(value);
    FLAG_B5 = BIT_TST5(value);
    FLAG_Z = result == 0;
    FLAG_S = result >> 7;
    */
    ...
    FLAG_N  = true;
    FLAG_V  = (((REG_A ^ value) & (REG_A ^ result)) & 0x80) != 0;
    FLAG_H  = ((REG_A ^ value ^ result) & 0x10) != 0;
    FLAG_B3 = (value >> 3) & 1;
    FLAG_B5 = (value >> 5) & 1;
    FLAG_Z  = result == 0;
    FLAG_S  = (result & 0x80) != 0;
...
static FORCE_INLINE uint16_t ADD_DD_16(struct SMS_Core *sms, uint16_t a, uint16_t b) {
    ...
    FLAG_B3 = BIT_TST3(result >> 8);
    FLAG_B5 = BIT_TST5(result >> 8);

    return result;
    */
    uint16_t result = a + b;

    FLAG_C  = result < a;  // Faster than (a + b) > 0xFFFF
    FLAG_N  = false;
    FLAG_H  = ((a ^ b ^ result) & 0x1000) != 0;  // Efficient 12-bit carry test
    FLAG_B3 = (result >> 11) & 1;
    FLAG_B5 = (result >> 13) & 1;
...
static FORCE_INLINE void ADC_hl(struct SMS_Core *sms, uint32_t value) {
    ...
    */
    ...
    FLAG_C  = result < hl;  // Carry out of 16-bit add
    FLAG_N  = false;
    FLAG_V  = ((hl ^ ~value) & (hl ^ result) & 0x8000) != 0;  // Overflow detection
    FLAG_H  = ((hl ^ value ^ result) & 0x1000) != 0;  // Half-carry from bit 11
    uint8_t hi = result >> 8;
    FLAG_B3 = (hi >> 3) & 1;
    FLAG_B5 = (hi >> 5) & 1;
    FLAG_Z  = result == 0;
...
*/
    ...
    FLAG_C  = result < HL; // carry out of 16-bit addition
    FLAG_H  = ((HL ^ value ^ result) & 0x1000) != 0; // half-carry from bit 11
    FLAG_N  = false;
...
static FORCE_INLINE uint8_t _INC(struct SMS_Core *sms, const uint8_t value) {
    ...
    */
    ...
    FLAG_N = false;
    FLAG_V = value == 0x7F;                     // Signed overflow check
    FLAG_H = (value & 0x0F) == 0x0F;            // Half-carry from bit 3 to 4
    FLAG_Z = result == 0;
    FLAG_S = result >> 7;
...
static FORCE_INLINE uint8_t qsSET(struct SMS_Core *sms, const uint8_t value, const uint8_t bit) {
    UNUSED(sms);
...
static FORCE_INLINE void CPI_CPD(struct SMS_Core *sms, int increment) {
    ...
    FLAG_B3 = BIT_TST3(b35_value);
    FLAG_B5 = BIT_TST1(b35_value);
    ...
    */
    ...
    // Precompute before modifying hl/bc
    bool half_carry = (REG_A & 0xF) < (value & 0xF);
    bool zero = (result == 0);
    bool sign = result & 0x80;  // Avoid shift
    bool parity = (--bc != 0);  // bc decremented once
    bool subtract = true;       // Always true

    hl += increment;

    // Precompute B3/B5 bits
    uint8_t adj = REG_A - value - half_carry;
    bool b3 = adj & 0x08;
    bool b5 = adj & 0x20;
...
static FORCE_INLINE void LDI_LDD(struct SMS_Core *sms, int increment) {
    ...
    FLAG_P = bc != 0;
    FLAG_N = false;
    FLAG_B3 = BIT_TST3(REG_A + value);
    FLAG_B5 = BIT_TST1(REG_A + value);
...
static FORCE_INLINE void DAA(struct SMS_Core *sms) {
    ...
    FLAG_Z = REG_A == 0;
    FLAG_S = REG_A >> 7;
    FLAG_B3 = BIT_TST3(REG_A);
    FLAG_B5 = BIT_TST5(REG_A);
}
...
static FORCE_INLINE bool _CB(struct SMS_Core *sms, const uint8_t opcode, const uint8_t value, uint8_t *result) {
    ...
    case 0x00:
        *result = RLC(sms, value);
        return true;
    case 0x01:
        *result = RRC(sms, value);
        return true;
    case 0x02:
        *result = RL(sms, value);
        return true;
    case 0x03:
        *result = RR(sms, value);
        return true;
    case 0x04:
        *result = SLA(sms, value);
        return true;
    case 0x05:
        *result = SRA(sms, value);
        return true;
    case 0x06:
        *result = SLL(sms, value);
        return true;
    case 0x07:
        *result = SRL(sms, value);
        return true;
    ...
    case 0x18: case 0x19: case 0x1A: case 0x1B: case 0x1C: case 0x1D: case 0x1E: case 0x1F:
        *result = qsSET(sms, value, 1 << ((opcode >> 3) & 0x7));
        return true;
    }
...
static FORCE_INLINE void execute_CB_IXIY(struct SMS_Core *sms, uint16_t ixy) {
    ...
    // so this this 'else' will be for bit!
        ...
        FLAG_B3 = BIT_TST3(addr >> 8);
        FLAG_B5 = BIT_TST5(addr >> 8);
        sms->cpu.cycles += 20;
    }
#endif

    const int8_t disp = (int8_t)read8(REG_PC++);
    const uint8_t opcode = read8(REG_PC++);
    const uint16_t addr = ixy + disp;
    const uint8_t value = read8(addr);
...
#define DISP() (int8_t)read8(REG_PC++)
    ...
    case 0x2E:
        *ixy_lo = read8(REG_PC++);
        break;
    case 0x24:
        *ixy_hi = _INC(sms, *ixy_hi);
        break;
    case 0x2C:
        *ixy_lo = _INC(sms, *ixy_lo);
        break;
    case 0x25:
        *ixy_hi = _DEC(sms, *ixy_hi);
        break;
    case 0x2D:
        *ixy_lo = _DEC(sms, *ixy_lo);
        break;
    case 0x84:
        _ADD(sms, *ixy_hi);
        break; // untested
    ...
    case 0x95:
        ...
        break; // untested
    case 0x96:
        _SUB(sms, read8(pair + DISP()));
        break;
    case 0x9E:
        _SBC(sms, read8(pair + DISP()));
        break;
    case 0xA4:
        _AND(sms, *ixy_hi);
        break; // untested
    ...
    case 0xCB:
        execute_CB_IXIY(sms, pair);
        break;

    default:
        dbug("UNK OP: 0xFD%02X\n", opcode);
        break;
    }
...
static FORCE_INLINE void execute_ED(struct SMS_Core *sms) {
    ...
    case 0x43:
        LD_imm_r16(sms, REG_BC);
        break;
    case 0x53:
        LD_imm_r16(sms, REG_DE);
        break;
    case 0x63:
        LD_imm_r16(sms, REG_HL);
        break;
    case 0x73:
        LD_imm_r16(sms, REG_SP);
        break;
    ...
    case 0x44: case 0x54: case 0x64: case 0x74: case 0x4C: case 0x5C: case 0x6C: case 0x7C:
        NEG(sms);
        break;
...
static FORCE_INLINE void execute(struct SMS_Core *sms) {
    ...
    case 0x41: case 0x42: case 0x43: case 0x44: case 0x45: case 0x46: case 0x47: case 0x48: case 0x4A: case 0x4B: case 0x4C: case 0x4D: case 0x4E: case 0x4F:
    case 0x50: case 0x51: case 0x53: case 0x54: case 0x55: case 0x56: case 0x57: case 0x58: case 0x59: case 0x5A: case 0x5C: case 0x5D: case 0x5E: case 0x5F:
    case 0x60: case 0x61: case 0x62: case 0x63: case 0x65: case 0x66: case 0x67: case 0x68: case 0x69: case 0x6A: case 0x6B: case 0x6C: case 0x6E: case 0x6F:
    case 0x70: case 0x71: case 0x72: case 0x73: case 0x74: case 0x75: case 0x77: case 0x78: case 0x79: case 0x7A: case 0x7B: case 0x7C: case 0x7D: case 0x7E:
        LD_rr(sms, opcode);
        break;
    ...
    case 0x20:
        JR_cc(sms, FLAG_Z == false);
        break;
    case 0x28:
        JR_cc(sms, FLAG_Z == true);
        break;
    case 0x30:
        JR_cc(sms, FLAG_C == false);
        break;
    case 0x38:
        JR_cc(sms, FLAG_C == true);
        break;

    case 0xC0:
        RET_cc(sms, FLAG_Z == false);
        break;
    case 0xC8:
        RET_cc(sms, FLAG_Z == true);
        break;
    case 0xD0:
        RET_cc(sms, FLAG_C == false);
        break;
    case 0xD8:
        RET_cc(sms, FLAG_C == true);
        break;
    case 0xE0:
        RET_cc(sms, FLAG_P == false);
        break;
    case 0xE8:
        RET_cc(sms, FLAG_P == true);
        break;
    case 0xF0:
        RET_cc(sms, FLAG_S == false);
        break;
    case 0xF8:
        RET_cc(sms, FLAG_S == true);
        break;

    case 0xC2:
        JP_cc(sms, FLAG_Z == false);
        break;
    case 0xCA:
        JP_cc(sms, FLAG_Z == true);
        break;
    case 0xD2:
        JP_cc(sms, FLAG_C == false);
        break;
    case 0xDA:
        JP_cc(sms, FLAG_C == true);
        break;
    case 0xE2:
        JP_cc(sms, FLAG_P == false);
        break;
    case 0xEA:
        JP_cc(sms, FLAG_P == true);
        break;
    case 0xF2:
        JP_cc(sms, FLAG_S == false);
        break;
    case 0xFA:
        JP_cc(sms, FLAG_S == true);
        break;

    case 0xC4:
        CALL_cc(sms, FLAG_Z == false);
        break;
    case 0xCC:
        CALL_cc(sms, FLAG_Z == true);
        break;
    case 0xD4:
        CALL_cc(sms, FLAG_C == false);
        break;
    case 0xDC:
        CALL_cc(sms, FLAG_C == true);
        break;
    case 0xE4:
        CALL_cc(sms, FLAG_P == false);
        break;
    case 0xEC:
        CALL_cc(sms, FLAG_P == true);
        break;
    case 0xF4:
        CALL_cc(sms, FLAG_S == false);
        break;
    case 0xFC:
        CALL_cc(sms, FLAG_S == true);
        break;
    ...
    case 0xCB:
        ...
        break;
    case 0xDD:
        execute_IXIY(sms, &REG_IXH, &REG_IXL);
        break;
    case 0xED:
        execute_ED(sms);
        break;
    case 0xFD:
        execute_IXIY(sms, &REG_IYH, &REG_IYL);
        break;
