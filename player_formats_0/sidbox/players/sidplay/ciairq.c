...
#define CR_TMR_INPUT_MASK   (3u << 5)
#define CR_TMR_PHI2         (0u << 5)
#define CR_TMR_CNT          (1u << 5)
#define CR_TMR_TA_UNDERFLOW (2u << 5)
#define CR_TMR_TA_UNDERFLOW_CNT (3u << 5)
...
void cia_write(cia_t *c, uint8_t reg, uint8_t v) {
    ...
    switch (reg) {
        ...
        case 0x00: c->pra = v; break;
        case 0x02: c->ddra = v; break;
...
static inline uint32_t cia_tick_timer_phi2(
    cia_t *c,
    uint16_t *t,
    uint16_t latch,
    uint8_t *cr,
    ...
    if (!(*cr & CRA_START)) return 0;

    uint32_t underflows = 0;

    while (cycles) {
        // NOTE: CIA timers effectively underflow after counting down through 0.
        // Your earlier "+1" version is usually closer than plain "t".
        uint32_t to_underflow = (*t == 0) ? 1u : ((uint32_t)(*t) + 1u);
...
void cia_tick(cia_t *c, uint32_t cycles) {
    // Timer A always counts PHI2 in your current model
    uint32_t ta_uf = cia_tick_timer_phi2(c, &c->ta, c->ta_latch, &c->cra, ICR_TA, cycles);

    // Timer B mode select (CRB bits 5..6)
    uint8_t tb_mode = (uint8_t)(c->crb & CR_TMR_INPUT_MASK);

    if (tb_mode == CR_TMR_PHI2) {
        // normal: TB counts PHI2 cycles
        (void)cia_tick_timer_phi2(c, &c->tb, c->tb_latch, &c->crb, ICR_TB, cycles);
    }
    else if (tb_mode == CR_TMR_TA_UNDERFLOW) {
        // chained: TB counts TA underflows (NOT cpu cycles!)
        if (ta_uf) {
            (void)cia_tick_timer_phi2(c, &c->tb, c->tb_latch, &c->crb, ICR_TB, ta_uf);
        }
    }
    else {
        // CNT / gated modes: not implemented yet.
        // safest behaviour for now: do nothing (prevents turbo).
        printf(".");
    }
}
