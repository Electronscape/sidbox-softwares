...
int paletteToloranceKnob = 3;   // this isused to adjust how hard the imported images match the existing colours
...
static const uint32_t PALETTE64_EDITOR[64] = {
    0x00000000, 0xFF000055, 0xFF0000AA, 0xFF0000FF, 0xFF005500, 0xFF005555, 0xFF0055AA, 0xFF0055FF, 0xFF00AA00, 0xFF00AA55, 0xFF00AAAA, 0xFF00AAFF, 0xFF00FF00, 0xFF00FF55, 0xFF00FFAA, 0xFF00FFFF,
    0xFF550000, 0xFF550055, 0xFF5500AA, 0xFF5500FF, 0xFF555500, 0xFF555555, 0xFF5555AA, 0xFF5555FF, 0xFF55AA00, 0xFF55AA55, 0xFF55AAAA, 0xFF55AAFF, 0xFF55FF00, 0xFF55FF55, 0xFF55FFAA, 0xFF55FFFF,
    0xFFAA0000, 0xFFAA0055, 0xFFAA00AA, 0xFFAA00FF, 0xFFAA5500, 0xFFAA5555, 0xFFAA55AA, 0xFFAA55FF, 0xFFAAAA00, 0xFFAAAA55, 0xFFAAAAAA, 0xFFAAAAFF, 0xFFAAFF00, 0xFFAAFF55, 0xFFAAFFAA, 0xFFAAFFFF,
    0xFFFF0000, 0xFFFF0055, 0xFFFF00AA, 0xFFFF00FF, 0xFFFF5500, 0xFFFF5555, 0xFFFF55AA, 0xFFFF55FF, 0xFFFFAA00, 0xFFFFAA55, 0xFFFFAAAA, 0xFFFFAAFF, 0xFFFFFF00, 0xFFFFFF55, 0xFFFFFFAA, 0xFFFFFFFF
};

uint32_t CLUTF_normal[256] = {
...
uint32_t CLUTF[256] = {
    0x00000000, 0xFFFF00FF, 0xFFFF00FF, 0xFFFF00FF, 0xFFFF00FF, 0xFFFF00FF, 0xFFFF00FF, 0xFFFF00FF, 0xFFFF00FF, 0xFFFF00FF, 0xFFFF00FF, 0xFFFF00FF, 0xFFFF00FF, 0xFFFF00FF, 0xFFFF00FF, 0xFFFF00FF,
    ...
    // greys
    0xFF101010, 0xFF383838, 0xFF606060, 0xFF888888, 0xFFB8B8B8, 0xFFFFFFFF,

    // reds
    0xFF240808, 0xFF4A1010, 0xFF7A1818, 0xFFA82828, 0xFFD86060, 0xFFF0B0B0,

    // oranges
    0xFF2A1408, 0xFF4A2410, 0xFF7A3C18, 0xFFAA5C20, 0xFFD89050, 0xFFF0C890,

    // browns / skin / earth
    0xFF1E140C, 0xFF3A2818, 0xFF5A3E28, 0xFF7A5638, 0xFFA87858, 0xFFD8B090,

    // yellows / olives
    0xFF2A2408, 0xFF4A4010, 0xFF6A5C18, 0xFF908020, 0xFFC8B840, 0xFFF0E090,

    // greens
    0xFF082008, 0xFF103810, 0xFF185818, 0xFF208020, 0xFF48B048, 0xFFA0E0A0,

    // teals
    0xFF082020, 0xFF103838, 0xFF185858, 0xFF208080, 0xFF48B0B0, 0xFFA0E0E0,

    // cyans / sky
    0xFF081828, 0xFF103048, 0xFF185878, 0xFF2080A8, 0xFF58B8D8, 0xFFB0E8F8,

    // blues
    0xFF081028, 0xFF102048, 0xFF183870, 0xFF2858A8, 0xFF6C90D8, 0xFFB8CCF8,

    // purples
    0xFF180820, 0xFF301040, 0xFF502070, 0xFF7038A0, 0xFFA070D0, 0xFFD8B8F0,

    // extras
    0xFFFF00FF, // debug / mask / obvious bad colour
    0xFFFF80C0, // pink accent
    0xFF80FFD0,  // mint accent
    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(ui->txtPaletteTolorancer, &QLineEdit::textChanged, this, [this](const QString &text){
        bool ok = false;
        uint32_t value = text.trimmed().toUInt(&ok);

        if (!ok)
            return;

        paletteToloranceKnob = value;
        if (paletteToloranceKnob < 1)
            paletteToloranceKnob = 1;
    });
    ...
    //connect(ui->txtPaletteSnapValue, &QTextBox)paletteToloranceKnob
...
static inline int biasedPaletteDistance(int sr, int sg, int sb, int pr, int pg, int pb, int tolerance)
{
    int dr = sr - pr;
    int dg = sg - pg;
    int db = sb - pb;

    // squash colour differences by tolerance
    dr /= tolerance;
    dg /= tolerance;
    db /= tolerance;
    ...
    const int srcLum = (sr * 30 + sg * 59 + sb * 11) / 100;
    const int palLum = (pr * 30 + pg * 59 + pb * 11) / 100;
    const int dLum = srcLum - palLum;
        ...
        for (int y = 0; y < h; y++) {
            ...
            for (int x = 0; x < w; x++) {
                ...
                for (int pi = palStart; pi < palEnd; pi++) {
                    ...
                    //int dist = dr*dr + dg*dg + db*db;
                    int dist = biasedPaletteDistance(rf, gf, bf, r2, g2, b2, paletteToloranceKnob);
