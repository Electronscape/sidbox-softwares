...
static void sega_mapper_update_slot1(struct SMS_Core* sms) {
    const uint32_t offset = 0x4000ul * sms->cart.mappers.sega.fffe;

    for (uint32_t i = 0; i < 0x10; ++i) {
        sms->cart.mappers.sega.banks[i + 0x10] = (const unsigned char *)ROM + offset + (0x0400ul * i);
    }
}

static void sega_mapper_update_slot2(struct SMS_Core* sms)
{
    const uint32_t offset = 0x4000ul * sms->cart.mappers.sega.ffff;

    for (uint32_t i = 0; i < 0x10; ++i)
    {
        sms->cart.mappers.sega.banks[i + 0x20] = (const unsigned char *)ROM + offset + (0x0400ul * i);
    }
}
...
static FORCE_INLINE void sega_mapper_write(struct SMS_Core* sms, uint16_t addr, unsigned char value)
{
    ...
    {
        ram[sms->cart.mappers.sega.fffc.ram_bank_select & 0x1][addr & 0x3FFF] = value;
    }
}
...
static inline void hi_ffxx_write(struct SMS_Core* sms, uint16_t addr, unsigned char value)
{
    ...
    case 0xFFFC: // Cartridge RAM mapper control
        ...
        else
        {
            // unamp ram
            sega_mapper_update_slot2(sms);
            //SMS_log("game is mapping romslot2 to rom region 0x8000!\n");
        }
        break;
...
void SMS_write_io(struct SMS_Core* sms, unsigned char addr, unsigned char value)
{
    ...
    // 0x07 and other odd addresses from 0x09 to 0x3F -> control write
    if ((addr == 0x07) || (addr >= 0x09 && addr <= 0x3F && (addr & 1))) {
        IO_control_write(sms, value);
        return;
    }

    // Even addresses from 0x08 to 0x3E -> memory control write
    if ((addr >= 0x08) && (addr <= 0x3E) && !(addr & 1)) {
        IO_memory_control_write(sms, value);
        return;
    }
    ...
    // No other ports handled in the original code
    /*
    switch (addr)
    {
        // GG regs
